    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-4 py-3 border-b border-white/5">
        <div className="flex items-center gap-2">
          <button
            onClick={onUpload}
            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white text-xs font-medium transition-all"
          >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-400 text-xs font-medium transition-all"
          >
        ...
        {queue.tracks.length === 0 ? (
          <div className="h-full flex items-center justify-center p-6">
            <div className="text-center">
              ...
              <p className="text-sm text-zinc-500 mb-1">Queue is empty</p>
              <p className="text-xs text-zinc-600">Add tracks to get started</p>
            </div>
          </div>
        ) : (
          <div className="py-2">
            {queue.tracks.map((track, index) => {
              ...
              return (
                <div
                  key={track.id}
                  className={cn(
                    'group flex items-center gap-3 px-4 py-2 hover:bg-white/5 transition-colors cursor-pointer',
                ...
                >
                  ...
                  <div className="w-6 flex items-center justify-center shrink-0">
                    <GripVertical className="w-4 h-4 text-zinc-600 hidden group-hover:block" />
                    <span className={cn(
                      'text-xs tabular-nums group-hover:hidden',
                      isCurrentTrack ? 'text-indigo-400' : 'text-zinc-600'
                    )}>
                      {index + 1}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className={cn(
                        'text-sm truncate',
                        isCurrentTrack ? 'text-white font-medium' : 'text-zinc-300'
                      )}>
                        {track.name}
                      </span>
                      {isUpNext && (
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div className={cn(
                      ...
                      track.youtubeId && 'bg-red-500/10 text-red-400',
                      track.aiGenerated && 'bg-purple-500/10 text-purple-400',
                      !track.youtubeId && !track.aiGenerated && 'bg-white/5 text-zinc-500'
                    )}>
                      {getSourceIcon(track)}
                    </div>
                  </div>
      ...
      {/* Queue Summary */}
      {queue.tracks.length > 0 && (
        <div className="px-4 py-2 border-t border-white/5 flex items-center justify-between text-xs text-zinc-500">
