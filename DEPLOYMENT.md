...
TURN servers relay media through firewalls when direct connections fail. Highly recommended for users on corporate networks.

1. In your Calls application settings, go to **TURN Configuration**
2. Enable **Cloudflare TURN**
3. Copy the TURN credentials:

```
TURN Server URL: turn:turn.cloudflare.com:3478
Username: (generated by Cloudflare)
Credential: (generated by Cloudflare)
```

4. Add to your environment variables:

```env
NEXT_PUBLIC_TURN_SERVER_URL=turn:turn.cloudflare.com:3478
NEXT_PUBLIC_TURN_USERNAME=your-turn-username
NEXT_PUBLIC_TURN_CREDENTIAL=your-turn-credential
```

**Note**: Cloudflare TURN credentials may be ephemeral (short-lived). For production, you may need to implement credential rotation via the Cloudflare API.
...
| `NEXT_PUBLIC_TURN_SERVER_URL` | `turn:turn.cloudflare.com:3478` (optional) |
| `NEXT_PUBLIC_TURN_USERNAME` | `your-turn-username` (optional) |
| `NEXT_PUBLIC_TURN_CREDENTIAL` | `your-turn-credential` (optional) |
| `CLOUDFLARE_R2_ACCOUNT_ID` | `your-account-id` |
